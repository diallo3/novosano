...
{	
	...
	define('MWB_WGM_HOME_URL', home_url());
	...
	add_action( 'init', 'mwb_wgm_insert_diwali_template');

	function mwb_wgm_insert_diwali_template(){
		if(isset($_GET['import_diwali_templates'])){
			$pagetemplate = get_option("mwb_wgm_new_diwali_template", false);
			if($pagetemplate == false){
				update_option("mwb_wgm_new_diwali_template", true);
				$filename = array( plugin_dir_path( __FILE__ )."assets/images/diwali.png");
	...
	function mwb_wgm_admin_settings($actions, $plugin_file) {
		...
		if ($plugin == $plugin_file) {
			$settings = array (
					'settings' => '<a href="' . home_url ( '/wp-admin/admin.php?page=mwb-wgc-setting' ) . '">' . __ ( 'Settings', 'woocommerce-ultimate-gift-card' ) . '</a>',
			);
