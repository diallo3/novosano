					...
					if($handle) 
					{
						...
						{	
							...
							if(isset($data)&&!empty($data)&&count($data)==5){
								...
								$postedValues[$count]['to'] = sanitize_text_field($data[0]);
								$postedValues[$count]['from'] = sanitize_text_field($data[1]);
					...
					<tbody>
						<tr>
							<th id="mwb_wgm_add_offline" colspan="2">
								...
								<a  href="<?php echo MWB_WGM_HOME_URL?>/wp-admin/admin.php?page=mwb-wgc-setting&tab=offline-giftcard" class="button button-primary"><?php _e("VIEW LIST", 'woocommerce-ultimate-gift-card');?>
							</th>
						</tr>
						<tr valign="top">
...
else
{
	...
	?>
		...
		<a id="mwb_wgm_add_new_card_button" class="page-title-action button button-primary button-large" href="<?php echo MWB_WGM_HOME_URL?>/wp-admin/admin.php?page=mwb-wgc-setting&tab=offline-giftcard&action=add"><?php _e('Add New', 'woocommerce-ultimate-gift-card');?></a>
	  	<form method="post">
