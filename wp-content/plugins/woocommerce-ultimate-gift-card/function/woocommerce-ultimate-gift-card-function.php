...
if( !class_exists( 'MWB_WGM_Card_Product_Function' ) )
{
	...
	class MWB_WGM_Card_Product_Function{
		...
		public function add_hooks_and_filters(){
			...
			add_filter( 'woocommerce_add_cart_item_data', array($this, 'mwb_wgm_woocommerce_add_cart_item_data'), 10, 2);
		...
		public function mwb_wgm_return_actual_price($price,$cart_item, $cart_item_key){
            ...
            if($product_type == 'wgm_gift_card')
            {
            	...
            	/*if ( 'excl' === WC()->cart->tax_display_cart && isset($test) && $test == 'no') {            		
            		return wc_price(($cart_item['line_subtotal'])/$cart_item['quantity']);       		
            	}
            	else if('excl' === WC()->cart->tax_display_cart && isset($test) && $test == 'yes'){
            		return wc_price(($cart_item['line_subtotal']+$cart_item['line_subtotal_tax'])/$cart_item['quantity']);  
            	}
            	else if('incl' === WC()->cart->tax_display_cart && isset($test) && $test == 'yes'){
            		return wc_price(($cart_item['line_subtotal']+$cart_item['line_subtotal_tax'])/$cart_item['quantity']);            		
            	}
            	else if('incl' === WC()->cart->tax_display_cart && isset($test) && $test == 'no'){
            		return wc_price(($cart_item['line_subtotal'])/$cart_item['quantity']);            		
            	}*/
            }
            else
							...
							if($this->mwb_wgm_create_gift_coupon($gift_couponnumber, $couponamont, $order_id, $item['product_id'],$to))
							{
								...
								if($expiry_date > 0 || $expiry_date === 0)
								{
									...
									if( isset($selected_date) && $selected_date !=null && $selected_date != "")
									{
										...
										$todaydate = date_create($todaydate);
										$todaydate = date_format($todaydate,$selected_date);
									}
									else
									{
								...
								update_post_meta($order_id, "$order_id#$item_id", $gift_couponnumber);
			...
			{	
				...
				if(isset(WC()->cart) && !empty(WC()->cart))
				{
					...
					{	
						...
						$product_types = wp_get_object_terms( $product_id, 'product_type' );
						//print_r($cart_item['data']->is_virtual());
						if(isset($product_types[0]))
						{
							$product_type = $product_types[0]->slug;
		...
		function mwb_wgm_preview_mail()
		{
			...
			if($browse_enable == "on"){
				...
				{
					if (!file_exists(wp_upload_dir()["basedir"].'/mwb_browse/'.$_FILES['file']['name'])){
						move_uploaded_file($_FILES['file']['tmp_name'], wp_upload_dir()["basedir"].'/mwb_browse/'.$_FILES['file']['name']);
					}					
				...
				if($product_type == 'wgm_gift_card')
				{
					...
					if($giftcard_expiry > 0)
					{
						...
						<div class="mwb_wgm_expiry_notice clear">
							...
							<p><?php echo sprintf(__('This gift card will expire after %s days after purchase.', 'woocommerce-ultimate-gift-card'), $days);?></p>
						</div>
						<?php 
					...
					else
					{
						...
						<div class="mwb_wgm_expiry_notice clear">
							<h4><?php _e("Giftcard Notice", 'woocommerce-ultimate-gift-card')?></h4>
							<p><?php echo sprintf(__("Giftcard has no expiry.", 'woocommerce-ultimate-gift-card'));?></p>
						</div>
						<?php 
					}	 
				}
		...
		function mwb_wgm_wp_enqueue_scripts()
		{
			...
			$mwb_wgm = array(
					...
					'discount_price_message'=>__('Need To Pay: ','woocommerce-ultimate-gift-card'),
					'coupon_message'=>__('Coupon Amount: ','woocommerce-ultimate-gift-card')
			);
			...
			{
				wp_register_script("mwb_wgm_product_single_script", MWB_WGM_URL."/assets/js/woocommerce-ultimate-gift-card-product-single.js", array('jquery'));
				wp_localize_script('mwb_wgm_product_single_script', 'mwb_wgm', $mwb_wgm );
				wp_enqueue_script('mwb_wgm_product_single_script' );
			}	
		}
						...
						if($product_type == 'wgm_gift_card')
						{
							...
							if(isset($product_pricing) && !empty($product_pricing))
							{
								if(isset($product_pricing['type']))
								{
									...
									{
										$price_html = apply_filters("mwb_wgm_user_price_text", __('Your Price.','woocommerce-ultimate-gift-card'));
									}
								}
							}
						}
						...
						if(isset($product_types[0]))
						{
							...
							if($product_type == 'wgm_gift_card')
							{
								...
								if(isset($product_pricing) && !empty($product_pricing))
								{
										...
										{	$default_price=$product_pricing['default_price'];
											...
											{?>
												<div class="mwb_wgm_price_content">
													...
													<b style="color:green;"><?php _e('Discount Price: ','woocommerce-ultimate-gift-card') ;echo $new_price_range;?></b><br/>
													<b style="color:green;"><?php _e('Coupon Amount: ','woocommerce-ultimate-gift-card') ;echo $text_box_price;?></b>
										...
										if($product_pricing_type == 'mwb_wgm_user_price')
										{
											...
											?>
												<div class="mwb_wgm_price_content">
													...
													<b style="color:green;"><?php _e('Discount Price: ','woocommerce-ultimate-gift-card') ;echo $new_price_user;?></b><br/>
													<b style="color:green;"><?php _e('Coupon Amount: ','woocommerce-ultimate-gift-card') ;echo $default_price;?></b>
									...
									?>
									<p class="mwb_wgm_section">
										<label><?php _e('From','woocommerce-ultimate-gift-card');?>:</label>	
									...
									<p>
										...
										<div class="mwb_wgm_delivery_method">
											...
											<input type="text"  name="mwb_wgm_to_email_name" id="mwb_wgm_to_download" class="mwb_wgm_to_email" placeholder="<?php _e('Enter the Recipient Name','woocommerce-ultimate-gift-card'); ?>"><span class= "mwb_wgm_msg_info"><?php echo $mwb_wgm_downloadable_desc;?></span>
										</div>
										<?php 
											}
										?>
										...
										<div class="mwb_wgm_delivery_method">
											...
											<input type="text"  name="mwb_wgm_to_email_ship" id="mwb_wgm_to_ship" class="mwb_wgm_to_email" placeholder="<?php _e('Enter the Recipient Name','woocommerce-ultimate-gift-card'); ?>"><span class= "mwb_wgm_msg_info"><?php echo $mwb_wgm_shipping_desc;?></span>
										</div>	
										<?php 
											}
										?>
											...
											if(isset($mwb_wgm_method_enable) && $mwb_wgm_method_enable == 'customer_choose')
											{
												if( isset($mwb_wgm_is_overwrite) && $mwb_wgm_is_overwrite == 'yes' )
												{
													...
													<div class="mwb_wgm_delivery_method">
														...
														<input type="text"  name="mwb_wgm_to_email_ship" id="mwb_wgm_to_ship" class="mwb_wgm_to_email mwb_wgm_disable" placeholder="<?php _e('Enter the Recipient Name','woocommerce-ultimate-gift-card'); ?>" readonly><span class= "mwb_wgm_msg_info"><?php echo $mwb_wgm_shipping_desc;?></span>
														</div>
													<?php
													}
												}
												else
												{
													...
													<div class="mwb_wgm_delivery_method">
														...
														<input type="text"  name="mwb_wgm_to_email" id="mwb_wgm_to_email" class="mwb_wgm_to_email"placeholder="<?php _e('Enter the Recipient Email','woocommerce-ultimate-gift-card'); ?>">
														<input type="text"  name="mwb_wgm_to_name_optional" id="mwb_wgm_to_name_optional" class="mwb_wgm_to_email"placeholder="<?php _e('Enter the Recipient Name (Optional)','woocommerce-ultimate-gift-card'); ?>">
														<span class= "mwb_wgm_msg_info"><?php echo $mwb_wgm_mail_to_recipient_desc ;?></span>
													</div>	
													<?php			
													}
									...
									?>
									<input name="add-to-cart" value="<?php echo $product_id; ?>" type="hidden">
									<?php if(is_array($templateid) && !empty($templateid)){?>
									<input name="mwb_wgm_selected_temp" id="mwb_wgm_selected_temp" value="<?php echo $templateid[0]; ?>" type="hidden">
									<?php 
									}
									if($mwb_wgm_preview_disable == 'off'){
									?>
									<br/><span class="price" ><a id="mwg_wgm_preview_email" href="javascript:void(0);"><?php _e('Preview','woocommerce-ultimate-gift-card');?></a> </span><?php }?>
									</p>
									</div>
									<?php
		 ...
		 * @link http://www.makewebbetter.com/
		 */
		function mwb_wgm_woocommerce_add_cart_item_data($the_cart_data, $product_id)
		{
					...
					if($product_type == 'wgm_gift_card')
					{
						if(isset($_POST['mwb_wgm_to_email']) && !empty($_POST['mwb_wgm_to_email']))
						{
							...
							if(isset($product_pricing) && !empty($product_pricing))
							{
								...
								$item_meta['mwb_wgm_to_email'] = $_POST['mwb_wgm_to_email'];
								$item_meta['mwb_wgm_from_name'] = $_POST['mwb_wgm_from_name'];
								$item_meta['mwb_wgm_message'] = $_POST['mwb_wgm_message'];
								...
								if($browse_img == "on")
								{
									...
									{
										if (!file_exists(wp_upload_dir()["basedir"].'/mwb_browse/'.$_FILES['mwb_wgm_browse_img']['name'])){
											move_uploaded_file($_FILES['mwb_wgm_browse_img']['tmp_name'], wp_upload_dir()["basedir"].'/mwb_browse/'.$_FILES['mwb_wgm_browse_img']['name']);
										}					
										
										$item_meta['mwb_wgm_browse_img'] = $_FILES['mwb_wgm_browse_img']['name'];
									} 
									
									
								}
								$item_meta = apply_filters('mwb_wgm_price_meta_data', $item_meta);
						...
						{	
							...
							if(isset($product_pricing) && !empty($product_pricing))
							{
								$item_meta['mwb_wgm_to_email'] = $_POST['mwb_wgm_to_email_name'];
								$item_meta['mwb_wgm_from_name'] = $_POST['mwb_wgm_from_name'];
								$item_meta['mwb_wgm_message'] = $_POST['mwb_wgm_message'];
								$item_meta['delivery_method'] = $_POST['mwb_wgm_send_giftcard'];
								$giftcard_selected_date = get_option("mwb_wgm_general_setting_enable_selected_date", false);
						...
						{	
							...
							if(isset($product_pricing) && !empty($product_pricing))
							{
								$item_meta['mwb_wgm_to_email'] = $_POST['mwb_wgm_to_email_ship'];
								$item_meta['mwb_wgm_from_name'] = $_POST['mwb_wgm_from_name'];
								$item_meta['mwb_wgm_message'] = $_POST['mwb_wgm_message'];
								$item_meta['delivery_method'] = $_POST['mwb_wgm_send_giftcard'];
								$giftcard_selected_date = get_option("mwb_wgm_general_setting_enable_selected_date", false);
				...
				if(isset($existing_item_meta ['product_meta']['meta_data']))
				{
					if ($existing_item_meta ['product_meta']['meta_data'])
					{
						foreach ($existing_item_meta['product_meta'] ['meta_data'] as $key => $val )
						{
							...
							{
								$item_meta [] = array (
										'name' => __('Original Price','woocommerce-ultimate-gift-card'),
										'value' => stripslashes( $val ),
								);
							}*/
						...
						foreach( $order->get_items() as $item_id => $item )
						{
							...
							if($mailsend)
							{
								...
								if($this->mwb_wgm_create_gift_coupon($gift_couponnumber, $couponamont, $order_id, $item['product_id'],$to))
								{
									...
									$args['expirydate'] = $expirydate_format;
									$args['amount'] =  wc_price($couponamont);
									$args['templateid'] = isset($selected_template) && !empty($selected_template) ? $selected_template : $templateid;
									//print_r($selected_template);die;
									$args['product_id'] = $product_id;
									$browse_enable = get_option("mwb_wgm_other_setting_browse", false);
									if($browse_enable == "on"){
										if($gift_img_name != ""){
